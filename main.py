...
'''
Original Source Code available in https://github.com/prashanth-up/Meet-Automation
# Originally Made By Prashanth Umapathy 
# Specialises in Extreme Laziness
...
def comment(name,regNo): #def comment(name,regNo,classSec):
    time.sleep(15)
    pag.press('tab')
    time.sleep(1)
    pag.press('tab')
...
def mainFunc():
    meeting_join()
    if(foo):
        comment(name,regNo) #comment(name,regNo,classSec)
